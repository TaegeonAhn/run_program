...
merged_prescript = pd.merge(df1,df2,left_on=['챠트번호','진료일자'], right_on=['챠트번호','발행일자'], how = 'outer')

#= pd.merge (prescription_data, prescription_slip_data, how = 'outer')

merged_prescript.to_csv('{}mm22.csv' .format(output_folder_directory), index=False, encoding='utf-8-sig')
